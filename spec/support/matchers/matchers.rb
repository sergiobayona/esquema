...
require "forwardable"
require_relative "matcher_factory"

MatcherFactory.new(:include_json).define_matcher do
  def traverse(expected, actual, negate = false)
    unless expected.is_a?(Hash) ||
           expected.is_a?(Array) ||
           expected.is_a?(::Matchers::UnorderedArrayMatcher)
      raise ArgumentError,
            "Expected value must be a json for include_json matcher"
    end

    representation = actual
    representation = JSON.parse(actual) if actual.is_a?(String)
...
MatcherFactory.new(:include_unordered_json).define_matcher do
  def traverse(expected, actual, negate = false)
    ...
    actual_json = actual
    actual_json = JSON.parse(actual) if actual.is_a?(String)
...
MatcherFactory.new(:match_unordered_json).define_matcher do
  def traverse(expected, actual, negate = false)
    unless expected.is_a?(Array)
      raise ArgumentError,
